...
#summary files that we use to create the figure
serrano_summary_mutfreqs_file = "files/serrano_summary_de_novo_mut_freqs.txt"

sc_summary_mutfreqs_file = "files/sc_summary_de_novo_mut_freqs.txt"
...
serrano_summary_mutfreqs = read.table(serrano_summary_mutfreqs_file, sep = '\t', header = TRUE)
sc_summary_mutfreqs = read.table(sc_summary_mutfreqs_file, sep = '\t', header = TRUE)
...
sc_study = sc_summary_mutfreqs %>%
  ...
  select(Tissue_Label, Age_Label, MUT_TYPE, AVG, SE, Study)
```

```{r}

serrano_df = serrano_summary_mutfreqs %>%
  #we only compare across B6 ancestry, which is what SC have in their study
  filter(STRAIN == "B6") %>%
  select(TISSUE, AGE_BIN, MUT_TYPE, AVG, SE) %>% 
  mutate(Study = "Serrano") %>%
  #recoding the brain label to match that of SC
...
study_freq_comp = ggplot(comp_df, aes(x = MUT_TYPE, y = AVG, fill = Study)) 
