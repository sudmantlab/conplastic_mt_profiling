...
Cluster 1: ALR cluster; measures ALR contamination 

Dummy FVB df to make sure we represent all samples including those without ALR contamination 
```{r}
SAMPLE = rep((unique(all_variants %>%
  filter(STRAIN == "F") %>% select(SAMPLE)))$SAMPLE, each = 2)

START = rep(c(4738, 9347), 24)

FVB_DUMMY_DF = cbind(data.frame(SAMPLE), data.frame(START))
```
...
cluster_1 = data.frame(START = c(4738, 9347), REF = c("C", "G"), ALT = c("A", "A"))
...
```{r}
imputed_cluster_1 = left_join(FVB_DUMMY_DF, cluster_1_cont, by = c("SAMPLE", "START")) %>%
...
fvb_cluster_1_cont = ggplot(imputed_cluster_1, aes(x = START, y = CONT_EST, color = CONT_FLAG)) + 
  geom_point(size = 0.7) + 
  geom_text(data = min_cont_cluster_1, aes(x = 7100, y = 0.001, label = formatC(MIN_CONT, digits = 2, format = "e")), size = 2, inherit.aes = FALSE) + 
...
pdf(paste(outdir_figures,"/fvb_cluster_1_cont.pdf",sep=""),width=7,height=5.5)
print(fvb_cluster_1_cont)
dev.off()

```

Cluster 2: NZB contamination 

```{r}
SAMPLE = rep((unique(all_variants %>%
  filter(STRAIN == "F") %>% select(SAMPLE)))$SAMPLE, each = 5)

START = rep(c(2765, 2766, 2797, 2813, 2839), 24)
...
```{r}
imputed_cluster_2 = left_join(FVB_DUMMY_DF, cluster_2_cont, by = c("SAMPLE", "START")) %>%
  select(SAMPLE, CONT_EST, CONT_FLAG) %>%
  mutate(CONT_EST = ifelse(is.na(CONT_EST), 0, CONT_EST)) %>%
  mutate(CONT_FLAG = ifelse(is.na(CONT_FLAG), "NO", CONT_FLAG))
...
fvb_cluster_2_cont = ggplot(imputed_cluster_2, aes(x = START, y = CONT_EST, color = CONT_FLAG)) + 
  geom_point(size = 0.7) + 
  geom_text(data = min_cont_cluster_2, aes(x = 2785, y = 0.00025, label = formatC(MIN_CONT, digits = 2, format = "e")), size = 2, inherit.aes = FALSE) + 
  facet_wrap(.~SAMPLE, nrow = 6) + 
  theme_bw() + 
  ylab("Contamination Est.") + 
  xlab("Haplotype site") + 
...
pdf(paste(outdir_figures,"/fvb_cluster_2_cont.pdf",sep=""),width=7,height=5.5)
print(fvb_cluster_2_cont)
dev.off()
...
```{r}
SAMPLE = rep((unique(all_variants %>%
  filter(STRAIN == "F") %>% select(SAMPLE)))$SAMPLE, each = 3)

START = rep(c(11842, 11845, 11932), 24)
...
```{r}
imputed_cluster_3 = left_join(FVB_DUMMY_DF, cluster_3_cont, by = c("SAMPLE", "START")) %>%
  select(SAMPLE, CONT_EST, CONT_FLAG) %>%
  mutate(CONT_EST = ifelse(is.na(CONT_EST), 0, CONT_EST)) %>%
  mutate(CONT_FLAG = ifelse(is.na(CONT_FLAG), "NO", CONT_FLAG))
...
fvb_cluster_3_cont = ggplot(imputed_cluster_3, aes(x = START, y = CONT_EST, color = CONT_FLAG)) + 
  geom_point(size = 0.7) + 
  geom_text(data = min_cont_cluster_3, aes(x = 11890, y =1e-5, label = formatC(MIN_CONT, digits = 2, format = "e")), size = 2, inherit.aes = FALSE) + 
  facet_wrap(.~SAMPLE, nrow = 6) + 
  theme_bw() + 
  ylab("Contamination Est.") + 
  xlab("Haplotype site") + 
...
pdf(paste(outdir_figures,"/fvb_cluster_3_cont.pdf",sep=""),width=7,height=5.5)
print(fvb_cluster_3_cont)
dev.off()
...
START = rep(c(15498, 15548, 15577, 15587, 15602), 24)
...
cluster_4 = data.frame(START = c(15498, 15548, 15577, 15587, 15602), REF = c("T", "C", "A", "C", "C"), ALT = c("A", "T", "T", "T", "T"))
...
```{r}
imputed_cluster_4 = left_join(FVB_DUMMY_DF, cluster_4_cont, by = c("SAMPLE", "START")) %>%
  select(SAMPLE, CONT_EST, CONT_FLAG) %>%
  mutate(CONT_EST = ifelse(is.na(CONT_EST), 0, CONT_EST)) %>%
  mutate(CONT_FLAG = ifelse(is.na(CONT_FLAG), "NO", CONT_FLAG))
...
fvb_cluster_4_cont = ggplot(imputed_cluster_4, aes(x = START, y = CONT_EST, color = CONT_FLAG)) + 
  geom_point(size = 0.7) + 
  geom_text(data = min_cont_cluster_4, aes(x = 15570, y =4e-4, label = formatC(MIN_CONT, digits = 2, format = "e")), size = 2, inherit.aes = FALSE) + 
  facet_wrap(.~SAMPLE, nrow = 6) + 
  theme_bw() + 
  ylab("Contamination Est.") + 
  xlab("Haplotype site") + 
...
pdf(paste(outdir_figures,"/fvb_cluster_4_cont.pdf",sep=""),width=7,height=5.5)
print(fvb_cluster_4_cont)
dev.off()
...
```{r}
write.table(cont_across_clusters_df, file = paste(outdir_files,"fvb_cluster_cont_est.txt", sep = ""), sep = "\t", quote = F, row.names = F)
...
```{r}
plotting_cont_range_fvb = cont_across_clusters_df %>%
  group_by(SAMPLE) %>%
...
```{r}
write.table(plotting_cont_range_fvb, file = paste(outdir_files,"fvb_sample_cont_est_range.txt", sep = ""), sep = "\t", quote = F, row.names = F)
...
fvb_cont_range = ggplot(plotting_cont_range_fvb, aes(x = AVG_CONT_CLUST, y = SAMPLE)) + 
  ...
  scale_color_manual(name = "Cluster", labels = c("cluster 1 (ALR)", "cluster 2 (NZB)", "cluster 3 (NZB)", "cluster 4 (NZB)"), values = c("#6e7cb9", "#7bbcd5", "#d0e2af","#f5db99")) + 
  theme_bw() + 
  xlab("Estimated Contamination") + 
  ylab("Sample")


pdf(paste(outdir_figures,"/fvb_cont_range.pdf",sep=""),width=7,height=5.5)
print(fvb_cont_range)
dev.off()
...
average_read_depth = read.table(average_read_depth_file, header=TRUE, stringsAsFactors = FALSE) %>%
  #the FVB strain is either F or FVB in files so we need to match here
  mutate(STRAIN = ifelse(STRAIN == "FVB", "F", STRAIN))

#chr1 NUMT contamination estimates to compare 
fvb_numt_corr_file = "/Users/isabelserrano/Documents/Science/Analyses/Conplastic_Strains/files_and_analyses/reversions/files/corrected_reversion_counts.txt"

fvb_numt_corr = read.table(fvb_numt_corr_file, header=TRUE, stringsAsFactors = FALSE) %>%
  filter(STRAIN == "FVB") %>%
...
```{r}
total_cont_FVB_df = cont_across_clusters_df %>%
  ungroup() %>%
...
```{r}
cond_fvb_est = total_cont_FVB_df %>%
...
```{r}
fvb_numt_corr_comp_df = rbind(fvb_numt_corr, cond_fvb_est)
```

Comparing the contamination estimate to the applied chr1 NUMT correction:

```{r}
fvb_numt_corr_comp = ggplot(fvb_numt_corr_comp_df, aes(x = COND_CONT_EST, y = COND_LABEL, color = TYPE)) + 
 geom_point() + 
  theme_bw() + 
  xlab("Condition Contamination \nEst.") + 
  ylab("Condition") + 
  scale_color_manual(name = "Measures", values = c("black", "magenta"), labels = c("Contamination Est", "NUMT correction factor")) + 
  theme(axis.text.x = element_text(angle = 90),
        axis.title.y = element_text(size = 13),
        legend.position = "none")

pdf(paste(outdir_figures,"/fvb_numt_corr_comp.pdf",sep=""),width=2.85,height=3)
print(fvb_numt_corr_comp)
dev.off()

pdf(paste(outdir_figures,"/leg_fvb_numt_corr_comp.pdf",sep=""),width=5,height=3)
print(fvb_numt_corr_comp  + theme(legend.position = "bottom") + guides(color =guide_legend(override.aes = list(size = 3)) ))
dev.off()
...
```{r}
write.table(cond_fvb_est, file = paste(outdir_files,"fvb_condition_cont_est.txt", sep = ""), sep = "\t", quote = F, row.names = F)
