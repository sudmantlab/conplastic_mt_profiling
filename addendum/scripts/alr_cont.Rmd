...
Cluster 1: Just one site -- FVB contamination

Dummy ALR df to make sure we represent all samples including those without FVB contamination 

```{r}
SAMPLE = (unique(all_variants %>%
  filter(STRAIN == "ALR") %>% select(SAMPLE))$SAMPLE)

START = rep(7777, 24)

ALR_DUMMY_DF = cbind(data.frame(SAMPLE), data.frame(START))
```
...
```{r}
imputed_cluster_1 = left_join(ALR_DUMMY_DF, cluster_1_cont, by = c("SAMPLE", "START")) %>%
...
alr_cluster_1_cont = ggplot(imputed_cluster_1, aes(x = START, y = CONT_EST, color = CONT_FLAG)) + 
  geom_point(size = 0.7) + 
...
pdf(paste(outdir_figures,"/alr_cluster_1_cont.pdf",sep=""),width=7,height=5.5)
print(alr_cluster_1_cont)
dev.off()
...
SAMPLE = rep((unique(all_variants %>%
  filter(STRAIN == "ALR") %>% select(SAMPLE)))$SAMPLE, each = 5)
...
```{r}
imputed_cluster_2 = left_join(ALR_DUMMY_DF, cluster_2_cont, by = c("SAMPLE", "START")) %>%
  select(SAMPLE, CONT_EST, CONT_FLAG) %>%
  mutate(CONT_EST = ifelse(is.na(CONT_EST), 0, CONT_EST)) %>%
  mutate(CONT_FLAG = ifelse(is.na(CONT_FLAG), "NO", CONT_FLAG))
...
alr_cluster_2_cont = ggplot(imputed_cluster_2, aes(x = START, y = CONT_EST, color = CONT_FLAG)) + 
  geom_point(size = 0.7) + 
  geom_text(data = min_cont_cluster_2, aes(x = 2785, y = 1e-4, label = formatC(MIN_CONT, digits = 2, format = "e")), size = 2, inherit.aes = FALSE) + 
  facet_wrap(.~SAMPLE, nrow = 6) + 
  theme_bw() + 
  ylab("Contamination Est.") + 
  xlab("Haplotype site") + 
...
pdf(paste(outdir_figures,"/alr_cluster_2_cont.pdf",sep=""),width=7,height=5.5)
print(alr_cluster_2_cont)
dev.off()
...
```{r}
SAMPLE = rep((unique(all_variants %>%
  filter(STRAIN == "ALR") %>% select(SAMPLE)))$SAMPLE, each = 3)
...
```{r}
imputed_cluster_3 = left_join(ALR_DUMMY_DF, cluster_3_cont, by = c("SAMPLE", "START")) %>%
  select(SAMPLE, CONT_EST, CONT_FLAG) %>%
  mutate(CONT_EST = ifelse(is.na(CONT_EST), 0, CONT_EST)) %>%
  mutate(CONT_FLAG = ifelse(is.na(CONT_FLAG), "NO", CONT_FLAG))
...
alr_cluster_3_cont = ggplot(imputed_cluster_3, aes(x = START, y = CONT_EST, color = CONT_FLAG)) + 
  geom_point(size = 0.7) + 
 geom_text(data = min_cont_cluster_3, aes(x = 11890, y =4e-5, label = formatC(MIN_CONT, digits = 2, format = "e")), size = 2, inherit.aes = FALSE) + 
  facet_wrap(.~SAMPLE, nrow = 6) + 
  theme_bw() + 
  ylab("Contamination Est.") + 
  xlab("Haplotype site") + 
...
pdf(paste(outdir_figures,"/alr_cluster_3_cont.pdf",sep=""),width=7,height=5.5)
print(alr_cluster_3_cont)
dev.off()
...
```{r}
imputed_cluster_4 = left_join(ALR_DUMMY_DF, cluster_4_cont, by = c("SAMPLE", "START")) %>%
  select(SAMPLE, CONT_EST, CONT_FLAG) %>%
  mutate(CONT_EST = ifelse(is.na(CONT_EST), 0, CONT_EST)) %>%
  mutate(CONT_FLAG = ifelse(is.na(CONT_FLAG), "NO", CONT_FLAG))
...
alr_cluster_4_cont = ggplot(imputed_cluster_4, aes(x = START, y = CONT_EST, color = CONT_FLAG)) + 
  geom_point(size = 0.7) + 
  geom_text(data = min_cont_cluster_4, aes(x = 15570, y =2e-4, label = formatC(MIN_CONT, digits = 2, format = "e")), size = 2, inherit.aes = FALSE) + 
  facet_wrap(.~SAMPLE, nrow = 6) + 
  theme_bw() + 
  ylab("Contamination Est.") + 
  xlab("Haplotype site") + 
...
pdf(paste(outdir_figures,"/alr_cluster_4_cont.pdf",sep=""),width=7,height=5.5)
print(alr_cluster_4_cont)
dev.off()
...
```{r}
write.table(cont_across_clusters_df, file = paste(outdir_files,"alr_cluster_cont_est.txt", sep = ""), sep = "\t", quote = F, row.names = F)
...
```{r}
plotting_cont_range_alr = cont_across_clusters_df %>%
  group_by(SAMPLE) %>%
...
```{r}
write.table(plotting_cont_range_alr, file = paste(outdir_files,"alr_sample_cont_est_range.txt", sep = ""), sep = "\t", quote = F, row.names = F)
...
alr_cont_range = ggplot(plotting_cont_range_alr, aes(x = AVG_CONT_CLUST, y = SAMPLE)) + 
  ...
  scale_color_manual(name = "Cluster", labels = c("cluster 1 (FVB)", "cluster 2 (NZB)", "cluster 3 (NZB)", "cluster 4 (NZB)"), values = c("#6e7cb9", "#7bbcd5", "#d0e2af","#f5db99")) + 
  theme_bw() + 
  xlab("Estimated Contamination") + 
  ylab("Sample")


pdf(paste(outdir_figures,"/alr_cont_range.pdf",sep=""),width=7,height=5.5)
print(alr_cont_range)
dev.off()
...
alr_numt_corr_file = "/Users/isabelserrano/Documents/Science/Analyses/Conplastic_Strains/files_and_analyses/reversions/files/corrected_reversion_counts.txt"

alr_numt_corr = read.table(alr_numt_corr_file, header=TRUE, stringsAsFactors = FALSE) %>%
  filter(STRAIN == "ALR") %>%
...
```{r}
total_cont_ALR_df = cont_across_clusters_df %>%
  ungroup() %>%
...
```{r}
cond_alr_est = total_cont_ALR_df %>%
...
```{r}
alr_numt_corr_comp_df = rbind(alr_numt_corr, cond_alr_est)
...
```{r}
alr_numt_corr_comp = ggplot(alr_numt_corr_comp_df, aes(x = COND_CONT_EST, y = COND_LABEL, color = TYPE)) + 
 geom_point() + 
  theme_bw() + 
...
pdf(paste(outdir_figures,"/alr_numt_corr_comp.pdf",sep=""),width=2.85,height=3)
print(alr_numt_corr_comp)
dev.off()

pdf(paste(outdir_figures,"/leg_alr_numt_corr_comp.pdf",sep=""),width=5,height=3)
print(alr_numt_corr_comp  + theme(legend.position = "bottom") + guides(color =guide_legend(override.aes = list(size = 3)) ))
dev.off()
...
```{r}
write.table(cond_alr_est, file = paste(outdir_files,"alr_condition_cont_est.txt", sep = ""), sep = "\t", quote = F, row.names = F)
