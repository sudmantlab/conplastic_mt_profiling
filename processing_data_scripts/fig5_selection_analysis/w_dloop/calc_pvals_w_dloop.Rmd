...
pdf(paste(outdir_figures,"/hnhs_per_gene_wo_hfps_w_dloop.pdf",sep=""), width=8,height=10.5)
...
p_values = z_scored_data %>% 
  ...
  #this comes from not dropping a sim condition but not having an obs (6 ratios are dropped)
  na.omit() %>%
...
pdf(paste(outdir_figures,"/summary_hnhs_per_gene_wo_hfps_w_dloop.pdf",sep=""), width=6,height=6)
print(summary_hNhS)
dev.off()

pdf(paste(outdir_figures,"/summary_hnhs_per_gene_wo_hfps_w_dloop_leg.pdf",sep=""), width=10,height=6)
...
write.table(p_values, file = paste(outdir_files,"/hNhS_per_gene_pvals_w_dloop.txt", sep = ""), sep = "\t", quote = F, row.names = F)
